...
#define COL_BG       (Color){ 12, 12, 12, 255 }      // Pure dark
#define COL_SIDEBAR  (Color){ 18, 18, 18, 255 }      // Slightly lighter dark
#define COL_BORDER   (Color){ 50, 50, 50, 255 }      // Subtle gray outline
#define COL_TEXT     (Color){ 240, 240, 240, 255 }   // Clean White Text
#define COL_TEXT_DIM (Color){ 140, 140, 140, 255 }   // Dim Gray Text
#define COL_NODE     (Color){ 12, 12, 12, 255 }      // Transparent/Dark center
#define COL_NODE_OUT (Color){ 100, 100, 100, 255 }   // Clean Gray border
#define COL_LOSS     (Color){ 220, 220, 220, 255 }   // White graph
#define COL_TEST     (Color){ 250, 204, 21, 255 }    // Accent Yellow for testing row
#define COL_POS      (Color){ 34, 197, 94, 255 }     // Positive = Clean Green
#define COL_NEG      (Color){ 239, 68, 68, 255 }     // Negative = Clean Red
#define COL_ACTIVATE (Color){ 255, 255, 255, 255 }   // Glow = Pure White
...
void DrawVisualization(NeuralNetwork* nn, AppState* state, const float* current_input, const float* current_output) {
    ...
    int num_layers = 6;
    int networkAreaWidth = state->screenWidth * 0.7f;
    float layerSpacing = (float)networkAreaWidth / (float)(num_layers + 1);
    float radius = 18.0f; // Leaner nodes

    // Helper for weights and neuron positions
    Vector2 positions[6];
    for (int l = 0; l < num_layers; l++) {
        positions[l] = (Vector2){ layerSpacing * (float)(l + 1), 0 };
    }
    ...
    for (int l = 0; l < num_layers; l++) {
        ...
        for (int n = 0; n < neuronsInLayer; n++) {
            ...
            DrawTextSmooth(state->mainFont, TextFormat("%.2f", activation), (int)x - 16, (int)y - 8, 14, (visual_act > 0.5f ? COL_BG : COL_TEXT));
            
            if (l == 0) {
                // Annotate the dynamic input features
                const char* featNames[] = {"EndsVow", "Ends 'A'", "Len > 5", "StrtVow", "Has 'Y'"};
                DrawTextSmooth(state->mainFont, featNames[n], (int)x - 90, (int)y - 8, 16, COL_TEXT_DIM);
            }
            if (l == num_layers - 1) { // Output layer
                float pred = nn->output_activations[0];
                // Render small numerical value next to node
                DrawTextSmooth(state->mainFont, TextFormat("%.2f", pred), x + 25, y - 10, 14, COL_POS);
                
                // Show Target during auto-training Evaluation Pass
                if (!state->custom_mode) {
                    DrawTextSmooth(state->mainFont, TextFormat("Target: %.2f", state->training_outputs[state->test_index][n]), (int)x + 35, (int)y + 5, 16, COL_TEST);
                }
            }
        }
    }
    ...
    // Progress Bar (Epochs approaching 3000)
    float progress = 0.0f;
    if (state->is_trained) progress = 1.0f;
    else {
        progress = (float)state->epoch / 3000.0f;
    ...
    DrawTextSmooth(state->mainFont, "> Arch: 5 -> 16 -> 12 -> 8 -> 4 -> 1", uiX + 20, 175, 16, COL_TEXT_DIM);
    
    // --- Live Sandbox UI ---
    DrawTextSmooth(state->mainFont, "--- Live Sandbox ---", uiX + 20, 215, 16, COL_TEXT_DIM);
    DrawTextSmooth(state->mainFont, "Type Name:", uiX + 20, 240, 14, COL_TEXT);
    DrawRectangle(uiX + 110, 235, 220, 26, COL_BG);
    DrawRectangleLines(uiX + 110, 235, 220, 26, state->custom_mode ? COL_ACTIVATE : COL_BORDER);
    DrawTextSmooth(state->mainFont, TextFormat("%s_", state->typed_name), uiX + 118, 240, 16, COL_TEXT);
    
    DrawTextSmooth(state->mainFont, "Extracted Features:", uiX + 20, 275, 14, COL_TEXT_DIM);
    DrawTextSmooth(state->mainFont, TextFormat("Ends Vowel: %1.0f", state->custom_inputs[0]), uiX + 20, 295, 14, state->custom_inputs[0] ? COL_POS : COL_TEXT_DIM);
    DrawTextSmooth(state->mainFont, TextFormat("Ends in A:  %1.0f", state->custom_inputs[1]), uiX + 170, 295, 14, state->custom_inputs[1] ? COL_POS : COL_TEXT_DIM);
    DrawTextSmooth(state->mainFont, TextFormat("Length > 5: %1.0f", state->custom_inputs[2]), uiX + 20, 315, 14, state->custom_inputs[2] ? COL_POS : COL_TEXT_DIM);
    DrawTextSmooth(state->mainFont, TextFormat("Starts Vow: %1.0f", state->custom_inputs[3]), uiX + 170, 315, 14, state->custom_inputs[3] ? COL_POS : COL_TEXT_DIM);
    DrawTextSmooth(state->mainFont, TextFormat("Has 'Y':    %1.0f", state->custom_inputs[4]), uiX + 20, 335, 14, state->custom_inputs[4] ? COL_POS : COL_TEXT_DIM);

    if (state->custom_mode) {
        float pred = nn->output_activations[0];
        const char* gender = pred > 0.5f ? "FEMALE" : "MALE";
        float conf = (pred > 0.5f ? pred : (1.0f - pred)) * 100.0f;
        DrawTextSmooth(state->mainFont, TextFormat("PREDICTION: %s (%.1f%%)", gender, conf), uiX + 20, 355, 16, pred > 0.5f ? COL_POS : COL_ACTIVATE);
    }

    // --- Dataset UI ---
    DrawTextSmooth(state->mainFont, "--- NLTK Names Dataset ---", uiX + 20, 380, 16, COL_TEXT_DIM);
    ...
    for (int i = 0; i < 9; i++) {
        ...
        DrawTextSmooth(state->mainFont, TextFormat("%-10s (%d%d%d%d%d) = %s", name, 
            ...
            uiX + 20, 405 + i * 18, 14, rowColor);
            
        if (!state->custom_mode && idx == state->test_index) DrawTextSmooth(state->mainFont, "<- TEST", uiX + 260, 405 + i * 18, 14, COL_TEST);
    }
    ...
    DrawTextSmooth(state->mainFont, "--- Training Loss ---", uiX + 20, 630, 16, COL_TEXT_DIM);
